    ...
    const [fieldData, setFieldData] = useState({
        user: {
            ...
            inquiryFields: {
                ...
                content: "문의 내용이 표시되는 영역입니다. 문의 내용이 표시되는 영역입니다. " + "문의 내용이 표시되는 영역입니다. 문의 내용이 표시되는 영역입니다.",
                attachments: ["filename.gif", "filename.gif", "filename.gif"],
                answer: "답변한 내용이 표시되는 영역입니다. 답변한 내용이 표시되는 영역입니다.답변한 내용이 표시되는 영역입니다.",
            ...
            if (field.type === 'date') {
                return (
                    <DateField
                        key={`${section}-${index}`}
                        label={field.label}
                        value={fieldData[site][section][field.key]}
                        width={field.width}
            ...
            if (field.type === 'checkbox') {
                return (
                    <CheckboxField
                        key={`${section}-${index}`}
                        label={field.label}
                        checked={fieldData[site][section][field.key]}
                        width={field.width}
                        onChange={(value) => handleDataChange(section, field.key, value)}
                    />
                );
            }
            if(field.type === 'imgList'){
                return (
                    <ImgList
                        key={`${section}-${index}`}
                        label={field.label}
        ...
        user: {
            ...
            refundFields: {
                ...
                data: [
                    ...
                    {label: "환불 계좌 정보", key: "accountInfo", width: "300px"},
                ]
            },
            inquiryFields: {
                label: "1:1 문의 정보",
                data: [
                    ...
                    {label: "문의 제목", key: "title",},
                    {label: "문의 내용", key: "content",},
                    {label: "첨부 파일", key: "attachments", type:"imgList"},
                    {label: "답변 내용", key: "answer",},
                    {label: "첨부 파일", key: "answerAttachments", type: "imgList"},
                ]
            },
        }
    }
        ...
        <div className={styles.container}>
            <main className={styles.content}>
                <header className={styles.header}>
                    <h1 className={styles.title}>{Const[site.toUpperCase()].title}</h1>
                ...
                {Object.keys(fieldModel[site]).map(it =>(
                    <section key={site+it}>
                    <h2 className={styles.sectionTitle}>{fieldModel[site][it].label}</h2>
                        {it === "adminMemo" ?
                            <textarea
                                className={styles.adminMemo}
                                value={fieldData[site].adminMemo}
                                onChange={(e) => handleDataChange('adminMemo', '', e.target.value)}
                                aria-label="관리자 메모"
                            />:null}
                        <div className={styles.fieldGrid}>
                            {renderFields(fieldModel[site][it].data, it)}
                        </div>
                    </section>
                ))}
                ...
                {/*        <div className={styles.contentText}>{fieldData.inquiry.title}</div>*/}
                {/*        <h3 className={styles.contentTitle}>문의 내용</h3>*/}
                {/*        <div className={styles.contentText}>{fieldData.inquiry.content}</div>*/}
                {/*        <h3 className={styles.contentTitle}>첨부 파일</h3>*/}
                {/*        <div className={styles.attachments}>*/}
                {/*            {fieldData.inquiry.attachments.map((file, index) => (*/}
                {/*                <div key={index} className={styles.file}>{file}</div>*/}
                {/*            ))}*/}
                {/*        </div>*/}
                {/*        <h3 className={styles.contentTitle}>답변 내용</h3>*/}
                {/*        <div className={styles.contentText}>{fieldData.inquiry.answer}</div>*/}
                {/*        <h3 className={styles.contentTitle}>첨부 파일</h3>*/}
                {/*        <div className={styles.attachments}>*/}
                {/*            {fieldData.inquiry.answerAttachments.map((file, index) => (*/}
                {/*                <div key={index} className={styles.file}>{file}</div>*/}
                {/*            ))}*/}
                {/*        </div>*/}
                {/*    </div>*/}
